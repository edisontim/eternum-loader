...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  onNotification: (callback: (notification: Notification) => void) => {
    const listener = (event: IpcRendererEvent, notification: Notification) =>
  ...
  onConfigChanged: (callback: (config: ToriiConfig) => void) => {
    const listener = (event: IpcRendererEvent, config: ToriiConfig) =>
      callback(config);
    ipcRenderer.on(IpcMethod.ConfigWasChanged, listener);
    return () =>
      ipcRenderer.removeListener(IpcMethod.ConfigWasChanged, listener);
  },
