...
export const Logs = () => {
  ...
  useEffect(() => {
    const removeListener = window.electronAPI.onNotification(
  ...
  return (
    page === Page.Syncing && (
      ...
      >
        ...
        <div className="flex flex-col w-full h-full p-2 gap-2 bg-white/50 backdrop-blur-md">
            ...
            {logs.map((log, index) => (
              <div
                ...
                className={`flex justify-between items-center rounded-sm text-base border-l-2 py-1 pl-3 pr-2 gap-2 ${
                  log.type === NotificationType.Error
                    ? "bg-deepRed/10 text-similiBlack border-l-deepRed/60"
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  {log.type === NotificationType.Error && (
                    <Error className="w-[16px] h-[16px] flex-shrink-0" />
                  )}
                  {log.message}
                </div>
                <div className="min-w-[125px] w-fit text-xs text-[#515151]/70">
